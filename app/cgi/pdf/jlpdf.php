...
class JLPDF extends FPDF
{
 ...
 //--------------- Imprimir un parrafo-----------
 function JLprint($cad, $ANCHO, $interlineado){  
	...
	//Texto justificado  
		...
		elseif($this->align=='c'){
			...
			if($i==count($lineasN)-1){// <- para la ultima línea
				//----rayitas
				$rayita='-';
				$ancho_rayita=$this->AnchoPalabra($rayita);
				//$un_espacio=' ';
				//$ancho_espacio=$this->AnchoPalabra($un_espacio);
				$espacio_sobrante=($ANCHO-$longitud);
				$cant_rayitas=($espacio_sobrante/$ancho_rayita);
				$r=0;
				for($r=0; $r<=($cant_rayitas-2); $r++){
				$rayitas=$rayitas.'-';
				}
				$espaciado=0;
			} 
			...
			for($ii=0; $ii<count($palabra);$ii++){
				...
				if(in_array($subpalabra,$this->comm)){ // Es comando?
					...
					if($i==count($lineasN)-1){// <- para la ultima línea
						$ultima_linea=true;
						$this->SetX($posicionX);
						$this->write($interlineado,"$subpalabra");
	...
	if ($ultima_linea==true){
		$this->SetX($posicionX);
		$this->write($interlineado,"$rayitas");
		//$posicionX+=($rayitas);
		$this->Ln(4);
	}
   $this->Ln();
 ...
 //--------------- Imprimir un texto-----------
 function JLCell($txt, $ANCHO, $alineacion, $interlineado)
 {
  ...
  for($i=0; $i<count($parrafos); $i++)
  {
    $this->SetX($posX);
    $this->SetFont($Auxfont,$AuxStyle);     // restore settings
    $this->SetFontSize(11); // restore settings  
    $this->TextColor($AuxColor['r'],$AuxColor['g'],$AuxColor['b']); // restore settings  
    
    $this->JLprint($parrafos[$i], $ANCHO,$interlineado);
